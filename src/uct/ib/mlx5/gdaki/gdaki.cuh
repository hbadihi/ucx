...
UCS_F_DEVICE ucs_status_t uct_rc_mlx5_gda_ep_single(
        ...
        uint64_t add, bool is_put_with_imm=false, uint32_t imm_data=0)
{
...
UCS_F_DEVICE int uct_rc_mlx5_gda_poll_recv_cq(uct_rc_gdaki_dev_ep_t *ep, uint32_t *signal_ptr)
{
    ...
    const unsigned long long print_interval = 10000000ULL;
    ...
    do
    {
        ...
        [[unlikely]] if (!is_ready && (++poll_count % print_interval) == 0) {
            printf("[Polling] Still waiting for CQE ready (idx=%u, polls=%llu)\n", 
                   idx, poll_count);
        }
    } while (!is_ready);
